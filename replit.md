...
-   **Technical Implementations**: Secure file uploads with Cloudinary signed URLs. Pydantic models strictly use `Field(default_factory=list/dict)` to prevent data leakage.
    ...
    -   **Login Page Branding (Dec 2025)**: Login page now displays tenant-specific branding (logo, website name, background image) from site settings. Public endpoint `/api/public/branding` fetches branding without authentication. Tenant slug is saved to localStorage on login and used for branding on subsequent visits.
    -   **Production-Ready URLs (Dec 2025)**: Removed all hardcoded localhost:8000 URLs. REST API uses relative "/api" path (or REACT_APP_BACKEND_URL env var). WebSocket URLs are dynamically constructed using `window.location.protocol` and `window.location.host`. Works universally on Replit, VPS, and production domains behind Nginx reverse proxy without code changes.
    -   **Dynamic Favicon (Dec 2025)**: Favicon is now configurable through Site Settings. Upload a custom favicon (supports .ico, .png, .jpg, .gif) in Settings > Branding. The favicon is fetched from settings and applied dynamically on both login page (via public branding API) and authenticated pages. Backend endpoint `/api/upload/favicon` handles upload with 1MB limit.
