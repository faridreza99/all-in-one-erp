...
const WarrantyDetails = () => {
  ...
  const handleCompleteInspection = async (e) => {
    ...
    if (
      !inspectionData.inspection_result ||
      inspectionData.inspection_result.length < 10
    ) {
      toast.error(
        "Please provide a detailed inspection result (minimum 10 characters)",
      );
      return;
    }
    ...
    try {
      ...
      await axios.post(
        `${API}/warranty/${warranty_id}/inspect`,
        {
          ...
          estimated_cost: inspectionData.estimated_cost
            ? parseFloat(inspectionData.estimated_cost)
            : null,
          attachments: [],
        },
        { headers: { Authorization: `Bearer ${token}` } },
      ...
      toast.error(
        error.response?.data?.detail || "Failed to complete inspection",
      );
    }
  };
  ...
  const handleSupplierAction = async (e) => {
    ...
    try {
      ...
      if (
        ...
        supplierActionData.replacement_serial
      ) {
        actionDetails.replacement_serial =
          supplierActionData.replacement_serial;
      }
      if (
        supplierActionData.action_type === "refund_issued" &&
        supplierActionData.refund_amount
      ) {
        actionDetails.refund_amount = parseFloat(
          supplierActionData.refund_amount,
        );
      }
      ...
      toast.error(
        error.response?.data?.detail || "Failed to record supplier action",
      );
    }
  };
  ...
  return (
    <>
      ...
      />
      <div
        className={`transition-all duration-300 p-6 space-y-6 ${sidebarCollapsed ? "lg:ml-2" : "lg:ml-80"}`}
      >
        {/* Header */}
        ...
        {/* Inspection Form */}
        {showInspectionForm &&
          warranty.current_status === "under_inspection" && (
            <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Inspection found it's the supplier's fault. Record the action
                  taken with the supplier.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSupplierAction} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  required
                >
                  <option value="replacement_sent">
                    Replacement Sent by Supplier
                  </option>
                  <option value="repair_sent">Repair Sent to Supplier</option>
                  <option value="cash_refund_offered">
                    Cash Refund Offered
                  </option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSupplierActionData({
                        ...supplierActionData,
                        replacement_serial: e.target.value,
                      })
                    }
              ...
              {(supplierActionData.action_type === "cash_refund_offered" ||
                supplierActionData.action_type === "partial_refund") && (
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">
                    Refund Amount (৳)
                  </label>
